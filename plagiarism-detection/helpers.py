"""
Add 'datatype' column that indicates if the record is original wiki answer as
0, training data 1, test data 2, onto the dataframe - uses stratified random
sampling (with seed) to sample by task & plagiarism amount
"""
import re
import operator
...
def create_datatype(df, train_value, test_value, datatype_var,
    ...
    # Random sample of subset dataframe to create new df with subset values
    df_sampled = df_subset.groupby(
        ['Task', compare_dfcolumn], group_keys=False).apply(
            lambda x: x.sample(
                min(len(x), sampling_number), random_state=sampling_seed
            )
        )
    ...
    # Adds test_value and train_value for all relevant data in subset dataframe
    for index in df_sampled.index:
        # Labels all datatype_var columns with test_value for test sample
    ...
    for index in df_subset.index:
        ...
        # based upon stratified test sample and subset of df
...
def train_test_dataframe(clean_df, random_seed=100):
    ...
    # creating a dictionary of categorical:numerical mappings for categories
...
def create_text_column(df, file_directory='data/'):
    '''Reads in the files, listed in a df and returns that df with an
